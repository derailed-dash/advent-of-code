...
-   **Performance:** `numpy` arrays are implemented in C and are much faster for numerical operations than standard Python lists.
-   **Convenience:** `numpy` provides a rich set of functions for array manipulation. Slicing, which we use extensively here, is incredibly powerful and concise.
-   **Broadcasting:** Although not used heavily in this specific solution, `numpy`'s broadcasting capabilities allow for efficient operations between arrays of different shapes and sizes.
...
-   **Applying `rotate` instructions**: Instead of converting to a list and back, we can use the `numpy.roll()` function. This function shifts the elements of an array along a given axis. This is a more direct and "NumPy-onic" way to perform the rotation.
...
![Display Animation](/assets/images/2016-08-animation.gif)
