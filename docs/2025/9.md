...
![Ray Casting Visualization](/assets/media/2025_d09_vis.gif)

The animation shows a test point (the dot) scanning across the polygon (using the real input shape).
- **Ray**: The dashed red line extending to the **left**.
- **Intersections**: The red `X` marks where the ray crosses a vertical edge **to the left** of the point.
- **Logic**:
    - **Odd** number of intersections (1, 3, etc.) $\rightarrow$ **Inside** (Green Dot).
    - **Even** number of intersections (0, 2, etc.) $\rightarrow$ **Outside** (Red Dot).
